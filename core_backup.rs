...
>>>>>>> 8d52ce1 (🏛️ Phase 1 Complete: Dependencies & Compilation Fixed)
...
<<<<<<< HEAD
        let output = self
            .model
            .forward(&token_ids.unsqueeze(0), Some(&attention_mask.unsqueeze(0)))?;
=======
...
>>>>>>> 8d52ce1 (🏛️ Phase 1 Complete: Dependencies & Compilation Fixed)
            ...
            // Forward pass
<<<<<<< HEAD
            let output = self.model.forward(&token_ids, Some(&attention_mask))?;

            // Mean pooling для каждого элемента батча
            for i in 0..batch_size {
                let pooled = output.i(i)?.mean(0)?;
=======
            let output = self.model.forward(&token_ids, &attention_mask, None)?;
