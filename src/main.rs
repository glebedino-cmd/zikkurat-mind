...
struct Args {
    ...
    /// Enable episodic memory (experimental)
    #[arg(long)]
    enable_memory: bool,
    ...
    /// Number of similar dialogues to retrieve (0 = disable)
    #[arg(long, default_value_t = 3)]
    memory_context_count: usize,
...
fn main() -> Result<()> {
    ...
    // Handle show_config flag (no prompt required)
    if args.show_config {
    ...
    // Handle save_config flag (no prompt required)
    if args.save_config {
        let config = load_config(args.config_file.as_deref())?;
        let effective_config = merge_config_with_args(config, &args);
